...
function spawnEnemy() {
    ...
    // **LOG für die Gegnergröße direkt nach der Instanziierung**
...
function Tick(deltaTime) {
    ...
    if (Date.now() - lastDifficultyIncrease > 10000) {
        difficulty += 1;
        enemySpawnRate = Math.max(500, enemySpawnRate - 200); // Spawn-Rate verk�rzen, min. 500ms
        console.log("Schwierigkeit erh�ht! Stufe:", difficulty, "Spawn-Rate:", enemySpawnRate);
    ...
    // Kollisionspr�fung: Kugeln vs Gegner
    for (let i = enemies.length - 1; i >= 0; i--) {
        ...
        if (distance < 15) { // Falls Spieler und Gegner sich ber�hren
            playerHP--;
            console.log("Spieler getroffen! HP:", playerHP);
            enemy.active = false; // Gegner verschwindet nach Ber�hrung
    ...
    for (let i = healItems.length - 1; i >= 0; i--) {
        ...
        if (distance < 15) { // Falls der Spieler das Item ber�hrt
...
function playSound(src) {
    ...
    sound.volume = 0.5; // Lautst�rke auf 50% setzen
    sound.play();
}